...
const LEVELS = [
  ...
  { id: 3, title: 'The Field', subtitle: 'O campo de jogo', flag: '🏟️', theme: 'from-emerald-500 to-teal-600', intro: 'Know every part of the pitch in English!',
    words: [{ en: 'pitch / field', pt: 'campo', ex: 'The pitch is in perfect condition.' }, { en: 'goal', pt: 'gol / baliza', ex: 'The ball hit the back of the goal.' }, { en: 'net', pt: 'rede', ex: 'The ball went into the net.' }, { en: 'post', pt: 'poste', ex: 'Her shot hit the post.' }, { en: 'penalty spot', pt: 'pênalti / marca do pênalti', ex: 'She stepped up to the penalty spot.' }, { en: 'penalty area', pt: 'área', ex: 'She was fouled inside the penalty area.' }, { en: 'center circle', pt: 'círculo central', ex: 'The game starts at the center circle.' }, { en: 'touchline', pt: 'linha lateral', ex: 'The ball went out on the touchline.' }, { en: 'corner flag', pt: 'bandeirinha do escanteio', ex: 'She ran to the corner flag to celebrate.' }, { en: 'bench', pt: 'banco (de reservas)', ex: 'The substitutes wait on the bench.' }] },
  ...
  { id: 5, title: 'On the Pitch', subtitle: 'Frases no campo', flag: '🗣️', theme: 'from-pink-500 to-rose-600', intro: 'Phrases you\'ll hear and use during training and matches!',
    words: [{ en: 'Man on!', pt: 'Marcação!', ex: '"Man on!" means there is a defender behind you.' }, { en: 'Well done!', pt: 'Muito bem!', ex: 'The coach said "Well done!" after the goal.' }, { en: 'Keep going!', pt: 'Continue! / Vai!', ex: '"Keep going!" encourages the team.' }, { en: 'Switch it!', pt: 'Muda o lado!', ex: '"Switch it!" means pass to the other side.' }, { en: 'Time!', pt: 'Tempo! (você tem espaço)', ex: '"Time!" means you have space to play.' }, { en: 'Press!', pt: 'Pressiona!', ex: '"Press!" means press the opponent now.' }, { en: 'Drop back!', pt: 'Recua!', ex: '"Drop back!" means defend deeper.' }, { en: 'Hold it!', pt: 'Segura a bola!', ex: '"Hold it!" means keep the ball.' }, { en: 'Shoot!', pt: 'Chuta!', ex: '"Shoot!" means take the shot now.' }, { en: 'Great teamwork!', pt: 'Ótimo trabalho em equipe!', ex: 'The coach praised the "great teamwork".' }] },
...
export default function EnglishGame() {
  ...
  const [miniFeedback, setMiniFeedback] = useState(null);
  const [speakingWord, setSpeakingWord] = useState(null);
  const [spokeFeedback, setSpokeFeedback] = useState({});
  const [flipped, setFlipped] = useState([]);
  const [matched, setMatched] = useState([]);
  const [spellingTarget, setSpellingTarget] = useState(null);
  ...
  const startGame = () => {
    const arr = [...level.words];
    for (let i = arr.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [arr[i], arr[j]] = [arr[j], arr[i]];
    }
    const pool = arr.slice(0, 6);
    setGamePool(pool); setGameIdx(0); setGameMatched([]); setScore(0); setView('game');
  };
  ...
  const handleGameNext = () => {
    if (gameIdx + 1 >= gamePool.length) {
      ...
      if (score >= 4) {
        ...
        if (nextUnlock > newProgress.unlockedLevel) {
          newProgress.unlockedLevel = nextUnlock;
        }
  ...
  const startMiniGame = (type) => {
    ...
    if (type === 'spelling') {
      const target = level.words[Math.floor(Math.random() * level.words.length)];
      setSpellingTarget(target);
    } else if (type === 'memory') {
      const wordList = level.words.slice(0, 4);
      const pairs = [...wordList, ...wordList].sort(() => Math.random() - 0.5);
          ...
          return (
            <motion.button key={lvl.id} disabled={locked} whileTap={locked ? {} : { scale: 0.97 }}
              ...
              className={`w-full text-left rounded-2xl border-2 p-4 flex items-center gap-3 transition-all ${
                locked ? 'opacity-40 cursor-not-allowed bg-muted/20 border-muted/30' : 'bg-gradient-to-r hover:shadow-lg border-border/30 hover:border-primary/50'
              }`}
              style={!locked ? { backgroundImage: `linear-gradient(to right, ${lvl.theme})20, transparent)` } : {}}>
  ...
  // === GAME (Jogos Mini-Quiz) ===
  if (view === 'game' && gamePool.length > 0 && !miniGame) {
    const current = gamePool[gameIdx];
    const otherWords = gamePool.filter(w => w.en !== current.en);
    const options = [...otherWords.slice(0, 2), current].sort(() => Math.random() - 0.5);
    ...
    return (
      <div className="space-y-4">
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          ...
          <button onClick={() => speak(current.pt)}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          <p className="text-xs text-muted-foreground mb-2">🎤 Bonus: Pronuncie a palavra!</p>
  ...
  // === MINI-GAMES ===
  if (view === 'game' && miniGame) {
    // Jogo 1: Spelling Challenge
    if (miniGame === 'spelling' && spellingTarget) {
      const target = spellingTarget;
      const letters = target.en.split('').sort(() => Math.random() - 0.5);
      ...
      const handleSpellingMic = () => {
        ...
        recognition.lang = 'en-US';
        recognition.interimResults = false;
        setMiniFeedback('🎤 Ouvindo...');
        try { recognition.start(); } catch (e) {}
        recognition.onresult = (event) => {
          const result = event.results?.[0]?.[0];
          if (!result) return;
          const transcript = (result.transcript || '').toLowerCase().trim();
          if (transcript === target.en.toLowerCase().trim()) {
            setMiniScore(miniScore + 1);
            setGameMatched([]);
            setMiniFeedback('✓ Perfeito!');
            recognition.abort();
            setTimeout(() => {
              if (miniScore + 1 >= 3) {
                setView('result');
                ...
                setSpellingTarget(level.words[Math.floor(Math.random() * level.words.length)]);
                setMiniFeedback(null);
              }
            }, 800);
          } else {
            setMiniFeedback(`Ouvido: "${transcript}"`);
            recognition.abort();
            setTimeout(() => setMiniFeedback(null), 2000);
          }
        };
        recognition.onerror = () => { recognition.abort(); setMiniFeedback('❌ Tente novamente'); };
      };
      ...
      return (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-bold text-primary">+{miniScore}</span>
          </div>
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
            className="bg-gradient-to-br from-purple-500/10 to-blue-500/10 border border-purple-500/20 rounded-3xl p-6 text-center">
            <p className="text-sm text-muted-foreground mb-2">Monte a palavra em inglês:</p>
            <p className="font-heading font-bold text-2xl text-primary mb-1">{target.pt}</p>
            <div className="flex gap-2 justify-center">
              <button onClick={() => speak(target.en)} className="flex items-center gap-2 text-blue-500 font-bold text-sm hover:text-blue-600">
                <Volume2 className="w-4 h-4"/> Ouvir
              </button>
              <button onClick={handleSpellingMic} className="flex items-center gap-2 text-green-500 font-bold text-sm hover:text-green-600">
                <Mic className="w-4 h-4"/> Falar
              </button>
            </div>
          </motion.div>
          <div className="flex flex-wrap gap-2 justify-center">
            {letters.map((l, i) => (
              <button key={i} onClick={() => {
                if (!gameMatched.includes(i)) {
                  const newMatched = [...gameMatched, i];
                  setGameMatched(newMatched);
                  if (newMatched.map(idx => letters[idx]).join('') === target.en) {
                    setMiniScore(miniScore + 1);
                    setGameMatched([]);
                    setMiniFeedback('✓');
                    setTimeout(() => setMiniFeedback(null), 800);
                  }
                }
              }} className="px-4 py-2 bg-primary text-primary-foreground rounded-xl font-bold">{l}</button>
            ))}
          </div>
          {miniScore >= 3 && <button onClick={() => { setView('result'); setMiniGame(null); }} className="w-full py-3 bg-green-500 text-white rounded-2xl font-bold">✓ Completado!</button>}
          {miniFeedback && (
            <motion.p initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-green-500 font-bold text-center">
              {miniFeedback}
            </motion.p>
          )}
        </div>
    ...
    // Jogo 2: Memory Match
    if (miniGame === 'memory' && memoryPairs.length > 0) {
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-2">
            {pairs.map((word, idx) => {
              ...
              return (
                <motion.button key={idx} whileTap={{ scale: 0.95 }} disabled={matched.includes(idx)}
                  onClick={() => {
                    if (!isFlipped && flipped.length < 2) {
                      const newFlipped = [...flipped, idx];
                      setFlipped(newFlipped);
                      if (newFlipped.length === 2) {
                        if (pairs[newFlipped[0]].en === pairs[newFlipped[1]].en) {
                          setTimeout(() => { setMatched([...matched, ...newFlipped]); setFlipped([]); }, 600);
                        } else {
                  ...
                  className={`aspect-square rounded-xl font-bold text-sm flex items-center justify-center transition-all ${isFlipped ? 'bg-primary text-white' : 'bg-muted border-2 border-border/30'}`}>
                  {isFlipped ? word.en : '?'}
                </motion.button>
              );
            })}
          </div>
          {matched.length === 8 && <button onClick={() => { setView('result'); setMiniGame(null); }} className="w-full py-3 bg-green-500 text-white rounded-2xl font-bold">✓ Completado!</button>}
        </div>
      );
    }
  }
    ...
    return (
      <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="space-y-5 text-center">
        ...
        <div className="bg-card border border-border/30 rounded-2xl p-4">
          <p className="text-5xl font-heading font-bold text-primary">{finalScore}/{gamePool.length}</p>
          <p className="text-sm text-muted-foreground mt-1">respostas corretas</p>
        </div>
        {!passed && (
          <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-2xl p-3 text-sm text-muted-foreground">
            Você precisa de 4+ respostas corretas! Revise o vocabulário e tente de novo.
          </div>
        )}
        {passed && (
          <div className="bg-green-500/10 border border-green-500/20 rounded-2xl p-3 text-center">
            <p className="text-sm font-bold text-green-700 dark:text-green-400 mb-3">🎮 Desafios Mini-Games!</p>
            <div className="flex gap-2">
              <button onClick={() => { setView('game'); startMiniGame('spelling'); }} className="flex-1 py-2 bg-purple-500 text-white rounded-xl font-bold text-sm">⌨️ Spelling</button>
              <button onClick={() => { setView('game'); startMiniGame('memory'); }} className="flex-1 py-2 bg-blue-500 text-white rounded-xl font-bold text-sm">🎮 Memory</button>
            </div>
          </div>
        )}
