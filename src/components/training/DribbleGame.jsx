...
export default function DribbleGame() {
  ...
  const [aimAngle,setAimAngle]=useState(-Math.PI/2); // direção do chute

  // refs — nunca causam re-render no loop
  ...
  const tlR    = useRef(60);
  const jvR    = useRef({x:0,y:0,on:false}); // joystick
  const angR   = useRef(-Math.PI/2);         // angulo
  ...
  const ballIdxR = useRef(0);

  const c = LVLS[lv];

  // Sincronizar state → ref sempre que mudar
  useEffect(() => { ownR.current = own; }, [own]);
  useEffect(() => { dizR.current = dizzy; }, [dizzy]);
  useEffect(() => { tlR.current = tl; }, [tl]);
  useEffect(() => { pgR.current = pg; }, [pg]);
  useEffect(() => { bgR.current = bg; }, [bg]);
  useEffect(() => { lvR.current = lv; }, [lv]);
  useEffect(() => { phR.current = ph; }, [ph]);
  useEffect(() => { ballIdxR.current = ballIdx; }, [ballIdx]);

  // ── Construir obstáculos ─────────────────────────────────────────────
  const buildObs = useCallback((lvlIdx) => {
  ...
  const resetRound = useCallback(() => {
    ...
    angR.current = -Math.PI/2; setAimAngle(-Math.PI/2);
  }, []);

  // ── Iniciar / reiniciar ─────────────────────────────────────────
  const startGame = useCallback((l) => {
    cancelAnimationFrame(rafR.current);
    clearInterval(tInt.current);
    phR.current = 'play'; setPh('play');
    lvR.current = l; setLv(l);
    pgR.current = 0; bgR.current = 0; setPg(0); setBg(0);
  ...
  const loop = useCallback(() => {
    ...
    const spd    = BALLS[ballIdxR.current].spd;
    const cfg    = LVLS[lvR.current];
    const isDizzy = dizR.current;
    const jv    = jvR.current;

    // — Player se move (se não tonto) ─────────────────────────────
    if (!isDizzy) {
      const np = {
        x: clamp(ppR.current.x + jv.x * PLAYER_SPEED * spd, PR, F_W - PR),
        y: clamp(ppR.current.y + jv.y * PLAYER_SPEED * spd, PR, F_H - PR),
      };
      ppR.current = np;
      setPp({...np});
      if (ownR.current === 'player') {
        bpR.current = {...np};
        setBp({...np});
      }
    }
    ...
    // — Bot se move sempre ────────────────────────────────────────
    const botTarget = ownR.current === 'bot'
      ? {x: F_W/2, y: F_H - 60}  // bot com bola → ataca gol de baixo
      : {x: F_W/2, y: 80};        // bot defendendo
    const bDx = botTarget.x - botR.current.x;
    const bDy = botTarget.y - botR.current.y;
    const bD  = Math.sqrt(bDx*bDx + bDy*bDy) || 1;
    const nb  = {
      x: clamp(botR.current.x + (bDx/bD)*cfg.botSpd*spd, BR, F_W-BR),
      y: clamp(botR.current.y + (bDy/bD)*cfg.botSpd*spd, BR, F_H-BR),
    };
    botR.current = nb;
    setBotPos({...nb});
    if (ownR.current === 'bot') {
      bpR.current = {x: nb.x, y: nb.y + 18};
      setBp({x: nb.x, y: nb.y + 18});
    }
    ...
    if (!isDizzy && ownR.current === 'player') {
      for (const o of obpR.current) {
        if (dist(ppR.current, {x:o.x, y:o.y}) < PR + 16) {
          ...
          setHitEff(true);
          setTimeout(() => { dizR.current = false; setDizzy(false); }, 1400);
    ...
    if (ownR.current === 'bot' && nb.y < G_H + 22 && Math.abs(nb.x - F_W/2) < G_W/2 + 5) {
      ownR.current = 'player'; setOwn('player');
      ...
      setTimeout(() => {
        setGoEff(false);
        if (bgR.current >= LVLS[lvR.current].goals) {
          setWinner('bot'); setPh('end'); phR.current = 'end';
        } else {
          resetRound();
          rafR.current = requestAnimationFrame(loop);
        }
    ...
    if (ownR.current === 'player' && ppR.current.y > F_H - G_H - 22 && Math.abs(ppR.current.x - F_W/2) < G_W/2 + 5) {
      ownR.current = 'bot'; setOwn('bot');
      ...
      setTimeout(() => {
        setGoEff(false);
        if (pgR.current >= LVLS[lvR.current].goals) {
          setWinner('player'); setPh('end'); phR.current = 'end';
        } else {
          resetRound();
          rafR.current = requestAnimationFrame(loop);
        }
  ...
  const joyEnd = () => {
    jcR.current = null;
    jvR.current = {x: 0, y: 0, on: false};
  };

  // ── Chute ─────────────────────────────────────────────────────
  const shoot = () => {
    if (ownR.current !== 'player' || dizR.current) return;
    ownR.current = 'bot'; setOwn('bot');
    setGoEff(true);
    ...
    return (
      <div className="flex flex-col items-center gap-5 py-4 px-4 min-h-[80vh]">
        <motion.div initial={{scale:.8,opacity:0}} animate={{scale:1,opacity:1}} className="text-center">
          ...
          <p className="text-xs text-muted-foreground">Drible, chute e marque gols!</p>
        </motion.div>
  ...
  // ── Jogo ─────────────────────────────────────────────────────
  const aimEndX = bp.x + Math.cos(aimAngle) * 50;
  const aimEndY = bp.y + Math.sin(aimAngle) * 50;
    ...
    <div className="flex flex-col items-center gap-2 select-none">
      ...
      <div className="relative rounded-2xl overflow-hidden shadow-2xl"
        ...
        <motion.div
          animate={{
            x: pp.x - PR,
            y: pp.y - PR,
            rotate: dizzy ? [0,20,-20,0] : 0,
          }}
          transition={dizzy ? { repeat: Infinity, duration: 0.2 } : { type:'spring',stiffness:300,damping:30 }}
          className={`absolute w-11 h-11 rounded-full border-2 shadow-lg flex items-center justify-center text-white font-black text-xs ${dizzy?'bg-yellow-400 border-orange-400 animate-pulse':own==='player'?'bg-blue-500 border-white':'bg-blue-300 border-white/50'}`}>
          {dizzy ? '💫' : '⚽'}
        </motion.div>
        ...
        {own === 'player' && !dizzy && (
          <svg width={F_W} height={F_H} className="absolute inset-0 pointer-events-none" style={{zIndex:5}}>
            <line
              ...
              stroke="#FFD600" strokeWidth="2.5" strokeDasharray="6 4" opacity="0.85"
            />
            <polygon
              points={`${aimEndX},${aimEndY} ${aimEndX + Math.cos(aimAngle+2.5)*10},${aimEndY + Math.sin(aimAngle+2.5)*10} ${aimEndX + Math.cos(aimAngle-2.5)*10},${aimEndY + Math.sin(aimAngle-2.5)*10}`}
      ...
      <div className="flex items-center gap-4 py-2">
        ...
        <div className="flex flex-col gap-2">
          ...
          <button onClick={() => { cancelAnimationFrame(rafR.current); clearInterval(tInt.current); setPh('menu'); }}
